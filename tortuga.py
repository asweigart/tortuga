...
# NOTE: Always have ASCII versions of languages come *after* (to the right of) the accented version of the language.
# We *want* the ASCII version of string values to be used in cases where it's ambiguous. For example, modo() is the
# Spanish and Spanish ASCII function name, but it should return 'normal_brujula'.
# This is caused by the fact that when modo() is called, we can't determine if the ASCII or accented version was intended.
# NOTE: Translated method names cannot collide with English names or else it causes a stack overflow. This
# is why, for example, French has mode_radians() instead of radians().
...
font  fuente  fuente  fonte fonte Schriftart  Schriftart
...
world mundo mundo monde monde Welt  Welt'''.splitlines()

# Parameter names and return values can collide with English names without a
# problem, so we list them here so they won't trigger an assertion later.
_TRANSLATION_NAME_COLLISION_EXEMPTIONS = ('orange', 'violet', 'magenta', 'cyan', 'triangle', 'angle', 'image', 'logo', 'normal')

_TRANSLATION_SPEED_ARGUMENTS = {
  'fastest':0, 'fast':10, 'normal':6, 'slow':3, 'slowest':1,  # Original English
'más rápida': 0, 'rápida': 10, 'normal': 6, 'lenta': 3, 'más lenta': 1,  # Spanish
'mas rapida': 0, 'rapida': 10, 'normal': 6, 'lenta': 3, 'mas lenta': 1,  # Spanish ASCII
'la plus rapide': 0, 'rapide': 10, 'normale': 6, 'lente': 3, 'la plus lente': 1,  # French and French ASCII
'schnellste': 0, 'schnelle': 10, 'normale': 6, 'langsame': 3, 'langsamste': 1,  # German and German ASCII
}

_TRANSLATION_SPREADSHEET_WRITE_FONT_ARGUMENTS = '''\
...
_TRANSLATION_WRITE_FONT_ARGUMENTS = {'normal':'normal', 'bold':'bold', 'italic':'italic', 'underline':'underline'}

_TRANSLATION_WRITE_ALIGN_ARGUMENTS = {'left':'left', 'right':'right'}

ALL_ENGLISH_IDENTIFIERS = [row.split()[0] for row in _TRANSLATION_SPREADSHEET]
...
def _create_translation_dictionaries():
    ...
    for i, row in enumerate(_TRANSLATION_SPREADSHEET):
        ...
        for j in range(1, _NUM_LANGUAGES):
            # If any non-English function names already have the same name as an English function, this will cause a stack overflow when calling the function.
            assert spreadsheet_columns[j] not in ALL_ENGLISH_IDENTIFIERS or spreadsheet_columns[j] in _TRANSLATION_NAME_COLLISION_EXEMPTIONS, spreadsheet_columns[j] + ' already exists as an English identifier'

        english_identifier = spreadsheet_columns[0]

        # ES ends up looking like {'bgcolor': 'color_fondo', 'bgpic': 'imagen_fondo', ... }
        ...
        if english_identifier == 'left':
            for j in range(1, _NUM_LANGUAGES):
                _TRANSLATION_WRITE_ALIGN_ARGUMENTS[spreadsheet_columns[j]] = 'left'
        elif english_identifier == 'right':
            for j in range(1, _NUM_LANGUAGES):
                _TRANSLATION_WRITE_ALIGN_ARGUMENTS[spreadsheet_columns[j]] = 'right'


    for i, row in enumerate(_TRANSLATION_SPREADSHEET_WRITE_FONT_ARGUMENTS):
        assert '-' not in row, 'Dash character found in spreadsheet row: ' + row
        assert "'" not in row, 'Quote character found in spreadsheet row: ' + row
        ...
        for j in range(1, _NUM_LANGUAGES):
            _TRANSLATION_WRITE_FONT_ARGUMENTS[spreadsheet_columns[j]] = english_identifier
...
_ALL_TRANSLATIONS_AND_VARS = ((ES, 'ES'), (ES_ASCII, 'ES_ASCII'),
                              (FR, 'FR'), (FR_ASCII, 'FR_ASCII'),
                              (DE, 'DE'), (DE_ASCII, 'DE_ASCII'))
...
for english_name in (_TG_SCREEN_FUNCTIONS + _TG_TURTLE_FUNCTIONS):
    ...
    for translation_mapping in _ALL_TRANSLATIONS:
        exported_nonenglish.append(translation_mapping[english_name])

exported_english = (_TG_CLASSES + _TG_SCREEN_FUNCTIONS + _TG_TURTLE_FUNCTIONS +
           _TG_UTILITIES + ('Terminator',))
__all__ = exported_english + tuple(set(exported_nonenglish))
...
class TurtleScreenBase(object):
    ...
    def _write(self, pos, txt, align, font, pencolor):
        ...
        # Tortuga translation of font and align arguments:
        if len(font) > 2:
            font = (font[0], font[1], ' '.join([_TRANSLATION_WRITE_FONT_ARGUMENTS[part] for part in font[2].split()]))
        align = _TRANSLATION_WRITE_ALIGN_ARGUMENTS[align]
...
class TPen(object):
    ...
    def speed(self, speed=None):
        ...
        speeds = _TRANSLATION_SPEED_ARGUMENTS
        if speed is None:
            return self._speed
        if speed in speeds:
            speed = speeds[speed]
...
def _make_translated_global_funcs(functions, cls, obj, init, docrevise, translation_mapping, translation_mapping_variable):
    ...
    for english_method_name in functions:
        ...
        #if nonenglish_method_name in ALL_ENGLISH_IDENTIFIERS:
        #  print('DEBUG: ' + str(nonenglish_method_name))
        #  continue
        ...
        """For example, these variables could have the following values:
        method == <function RawTurtle.write at 0x103be8f40>
        english_method_name == 'write'
        english_pl1 == "(arg, move=False, align='left', font=('Arial', 8, 'normal'))"
        english_pl2 == '(arg, move, align, font)'
        nonenglish_pl1 == "(arg, mover=False, alinear='izquierda', fuente=('Arial', 8, 'normal'))"
        nonenglish_pl2 == '(arg, mover, alinear, fuente)'
        defstr == '''
        def escribir(arg, mover=False, alinear='izquierda', fuente=('Arial', 8, 'normal')):
            if Turtle._pen is None:
                ...
                Turtle._pen = Turtle()
            try:
                arg = _ES_INVERSE.get(arg, arg)
                mover = _ES_INVERSE.get(mover, mover)
                alinear = _ES_INVERSE.get(alinear, alinear)
                fuente = _ES_INVERSE.get(fuente, fuente)
                result = write(arg, mover, alinear, fuente)
                return ES.get(result, result)
            except TK.TclError:
                if not TurtleScreen._RUNNING:
                    TurtleScreen._RUNNING = True
                    raise Terminator
        ...
        #if nonenglish_method_name == 'escribir':
        #    print(defstr)
        #    input('>>>>>')
...
for noneng_map, noneng_map_var in _ALL_TRANSLATIONS_AND_VARS:
